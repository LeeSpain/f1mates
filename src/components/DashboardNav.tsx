...
export const DashboardNav = ({ activeTab, setActiveTab }: DashboardNavProps) => {
  ...
  return (
    <nav className="w-64 bg-sidebar pt-6 border-r border-f1-silver/20 hidden md:block">
      <div className="px-3 mb-6">
        <div className="text-primary font-bold text-lg px-4">F1 Mate Racer</div>
      </div>
      ...
      <div className="mt-auto px-3 pb-6 pt-6">
        <div className="rounded-md bg-sidebar-accent p-4">
          <h3 className="font-medium text-sm text-sidebar-foreground mb-2">Next Deadline</h3>
          <p className="text-xs text-sidebar-foreground/80">Submit picks by Thursday before the Saudi Arabia GP</p>
        </div>
      </div>
    </nav>
  );
